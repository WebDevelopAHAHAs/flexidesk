...
export default function Wrapper(props) {
  let details = null
  return(<ModalWrap dataType="desk" formType="add" header="Add Desk" id={props.userID} details={details}/>)
}

export function Button(props) {
  return ( <button id="AddDesk" onClick={props.handleOpen}><span>Add Desk</span></button> )
}
...
export function Layout(props) {
  ...
  function handleChange(event) {
    ...
    if(name === "available") {
      console.log("Changed to bool hit")
  ...
  function handleSubmit(event) {
      ...
      createDesk(deskDetails).then(() => {
        dispatch({ type: "newDesk", data: deskDetails });
